...
export const PlayerGameView = ({userObj, gameStarted, playerView, setPlayerView}) => {
    ...
    const draggingChips = (eX, eY, chipAmount) => {
        if (eX > 40 && eX < 315) {
            if (eY > 313 && eY < 455) {
                user.playerGameObj.betAmount += chipAmount
                setPlayerBetAmount(user.playerGameObj.betAmount);
            }
        }
    }
    ...
    return (
        <View style={{flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: 'mistyrose', borderColor: 'lightgrey', width: '100%', height: '100%', display: gameStarted === true && playerView === false ? 'flex' : 'none', position: 'absolute'}}>
            ...
            <View style={{borderWidth: 4, borderRadius: 5, width: '100%', height: '30%', backgroundColor: 'papayawhip', position: 'absolute', top: '28%', justifyContent: 'center'}}>
                ...
                <View style={{borderWidth: 2, borderRadius: 5, backgroundColor: 'lightgrey', width: '30%', position: 'absolute', top: 50, alignSelf: 'center'}}>
                    <Button 
                        title='Check'
                        color='black'
                        onPress={() => user.playerGameObj.checks()}
                    />
                </View>
            </View>
            ...
            <View style={{borderWidth: 4, borderRadius: 5, backgroundColor: 'papayawhip', position: 'absolute', top: '77%', width: '100%', height: '10%'}}>
                <Draggable x={10} y={-38} onDragRelease={(e) => draggingChips(e.nativeEvent.pageX, e.nativeEvent.pageY, 0.25)} shouldReverse={true}>  
                ...
                <Draggable x={100} y={-38} shouldReverse={true}>  
                ...
                <Draggable x={190} y={-38} shouldReverse={true}>  
                ...
                <Draggable x={280} y={-38} shouldReverse={true}>  
